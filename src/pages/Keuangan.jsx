    ...
    const handleSubmit = async (e) => {
        ...
        types.forEach((type, index) => {
            ...
            if (valStr && valStr !== '0' && valStr !== '') {
                // If emas, clean is strict float, else integer only
                ...
                if (cleanAmount > 0) {
                    newRecords.push({
                        // Temp ID for optimistic update (timestamp + index to avoid collision)
                        id: timestamp + index,
    ...
    return (
        <div className="keuangan-container">
            ...
            <div className="grid-stats">
                ...
                <StatCard title="Emas" value={stats.totalEmasValue} sub={`${stats.totalEmasGrams} Gr`} icon={<Coins size={20} color="#facc15" />} color="rgba(250, 204, 21, 0.1)" />
                    ...
                    <table className="trx-table">
                        ...
                        <tbody>
                            {filteredTransactions.map(t => (
                                <tr key={t.id}>
                                    <td style={{ fontWeight: '500', color: '#cbd5e1' }}>{new Date(t.date).toLocaleDateString('id-ID', { day: '2-digit', month: 'short' })}</td>
                                    <td>
                                    ...
                                    <td style={{ textAlign: 'center' }}>
                                        <button onClick={() => handleDelete(t.id)} style={{ background: 'transparent', border: 'none', color: '#64748b', cursor: 'pointer', padding: '4px' }}><Trash2 size={16} /></button>
                                    </td>
                                </tr>
                            ))}
                        </tbody>
                        ...
                        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="modal-content">
                            ...
                            <form onSubmit={handleSubmit}>
                                ...
                                <div className="batch-grid">
                                    ...
                                    {/* EMAS */}
                                    <div className="form-group">
                                        <label className="form-label">🥇 Beli Emas <span style={{ color: '#facc15' }}>GRAM</span></label>
